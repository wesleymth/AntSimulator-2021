...
Pheromone::Pheromone()
    :Pheromone(Vec2d(getAppConfig().world_size/2,getAppConfig().world_size/2), getAppConfig().DEFAULT_PHEROMONE_QUANTITY)
{
    //Done
}
...
{
    evaporate(dt.asSeconds()*getAppConfig().pheromone_evaporation_rate*getAppEnv().getTemperature()/getAppConfig().temperature_initial);
}
